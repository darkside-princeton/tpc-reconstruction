// This file can be used to plot laser runs where the sync waveform is in
// channel 0
...
void PlotLaserData() {
  ...
  TFile *f = new TFile("FinalData/Laser070821/Laser44v_070821.root");
  TTree *tPulse = (TTree*)f->Get("Pulse_Tree");

  TGraph* avg_pulse = (TGraph*)f->Get("gAvgSPEPulse_ch1");
  ...
  TH2D *hLengthIntegral = new TH2D("hLengthIntegral","Length vs. Charge; Pulse Length [#mus]; Charge [ADC Counts]",125,0,0,125,0,0);

  //Pulses with afterpulses
  TH1D *hStartTime_ap   = new TH1D("hStartTime_ap","Start Time Distribution; Time [#mus]; Counts",300,0,10);
  TH1D *hLength_ap   = new TH1D("hLength_ap","Pulse Length Distribution; Time [#mus]; Counts",100,0,1);
  TH1D *hIntegral_ap   = new TH1D("hIntegral_ap","Charge Distribution; Charge [ADC Counts]; Counts",300,0,0);
  TH1D *hHeight_ap   = new TH1D("hHeight_ap","Height Distribution; Height [ADC Counts]; Counts",300,0,0);
  TH2D *hHeightIntegral_ap = new TH2D("hHeightIntegral_ap","Height vs. Charge; Height [ADC Counts]; Charge [ADC Counts]",300,0,0,300,0,0);
  TH2D *hLengthIntegral_ap = new TH2D("hLengthIntegral_ap","Length vs. Charge; Pulse Length [#mus]; Charge [ADC Counts]",125,0,0,125,0,0);

  //After-pulse Plots
  TH1D *hAPTimeDiff = new TH1D("hAPTimeDiff","Afterpulse Time Distribution; Time Since Previous Pulse [#mus]; Counts",100,0,10);
  TH1D *hAPHeight = new TH1D("hAPHeight","Afterpulse Height Distribution; Height [ADC Counts]; Counts",100,0,0);
  TH2D *hAPTimeHeight = new TH2D("hAPTimeHeight","Afterpulse Time vs. Height Distribution; Time Since Previous Pulse [#mus]; Height [ADC Counts]",500,0,0,500,0,0);

  //Other
  TH1D *hTimeRes   = new TH1D("hTimeRes","Time Resolution (SPEs); Reconstructed Time - Start Time [#mus]; Counts",75,-0.1,0.2);
  TH1D *hTimeResAll   = new TH1D("hTimeResAll","Time Resolution (All Pulses) ; Reconstructed Time - Start Time [#mus]; Counts",75,-0.1,0.2);
  TH2D *hTimeResHeight   = new TH2D("hTimeResHeight","Time Resolution vs. Height; Pulse Height [ADC Counts]; Reconstructed Time - Start Time [#mus]",75,0,450, 75, -0.1, 0.2);
  ...
  for (Int_t i = 0; i < nentries; i++) {
    ...
    for(int p = 0; p < npulses[1]; p++) { // for each of the pulses in the event
      ...
      //Filling time resolution plots
      if(p == 0 && npulses[0] > 0 && height[0]->at(0) > 1000 && height[1]->at(0) > 20 && start_time[1]->at(0) > 1.8 && start_time[1]->at(0) < 2) {
        if(integral[1]->at(0) > 200 && integral[1]->at(0) < 500) hTimeRes->Fill(half_time[1]->at(0) - half_time[0]->at(0));
        hTimeResAll->Fill(half_time[1]->at(0) - half_time[0]->at(0));
        hTimeResHeight->Fill(height[1]->at(0), half_time[1]->at(0) - half_time[0]->at(0));
      }

      // Pulses without afterpulses
      if(nafterpulses[1]->at(p) == 0 && p == 0) {
        hStartTime->Fill(start_time[1]->at(p));
        hLength->Fill(end_time[1]->at(p) - start_time[1]->at(p));
        hIntegral->Fill(integral[1]->at(p));
        hHeight->Fill(max_val[1]->at(p));
        hHeightIntegral->Fill(max_val[1]->at(p), integral[1]->at(p));
        hLengthIntegral->Fill(end_time[1]->at(p) - start_time[1]->at(p), integral[1]->at(p));
      }

      // Pulses with afterpulses
      if(nafterpulses[1]->at(p) > 0) {
        hStartTime_ap->Fill(start_time[1]->at(p));
        hLength_ap->Fill(end_time[1]->at(p) - start_time[1]->at(p));
        hIntegral_ap->Fill(integral[1]->at(p));
        hHeight_ap->Fill(height[1]->at(p));
        hHeightIntegral_ap->Fill(height[1]->at(p), integral[1]->at(p));
        hLengthIntegral_ap->Fill(end_time[1]->at(p) - start_time[1]->at(p), integral[1]->at(p));
      }

      // Afterpulses within a pulse
      for(int j = 0; j < ap_peak_times[1]->at(p).size(); j++) {
        if(j == 0 && start_time[1]->at(0) > 1.8) {
          hAPTimeDiff->Fill(ap_peak_times[1]->at(p).at(j) - peak_time[1]->at(p));
          hAPTimeHeight->Fill(ap_peak_times[1]->at(p).at(j) - peak_time[1]->at(p), ap_heights[1]->at(p).at(j));
        }
        else if (j > 0 && start_time[1]->at(0) > 1.8) {
          hAPTimeDiff->Fill(ap_peak_times[1]->at(p).at(j) - peak_time[1]->at(p));
          hAPTimeHeight->Fill(ap_peak_times[1]->at(p).at(j) - peak_time[1]->at(p), ap_heights[1]->at(p).at(j));
        }
        hAPHeight->Fill(ap_heights[1]->at(p).at(j));
      }
      if(p > 0 && start_time[1]->at(0) > 1.8) {
        hAPHeight->Fill(height[1]->at(p));
        if (nafterpulses[1]->at(p-1) == 0) {
          hAPTimeDiff->Fill(peak_time[1]->at(p) - peak_time[1]->at(p-1));
          hAPTimeHeight->Fill(peak_time[1]->at(p) - peak_time[1]->at(p-1), height[1]->at(p));
        }
        if (nafterpulses[1]->at(p-1) > 0) {
          hAPTimeDiff->Fill(peak_time[1]->at(p) - ap_peak_times[1]->at(p-1).at(ap_peak_times[1]->at(p-1).size()-1));
          hAPTimeHeight->Fill(peak_time[1]->at(p) - ap_peak_times[1]->at(p-1).at(ap_peak_times[1]->at(p-1).size()-1), height[1]->at(p));
        }
      }
    }
  }
  ...
  TF1* fTimefit = new TF1("fTimefit", "[0] + exp([1] + [2]*x) +  exp([3] +[4] * x)");
  fTimefit->SetParameters(130, 8, -10, 7, -1.5);
  c2->cd(2); hAPTimeDiff->Fit("fTimefit", "", "", 0.1, 7.3); hAPTimeDiff->Draw();
 ...
 c3->Update();
  ...
  hFFT = havgPulse->FFT(hFFT, "MAG");
  TH1D *h1dMag = new TH1D("h1dMag", "FFT of Average Pulse", avg_pulse->GetN()/2, 0, 0.25*1e3/2);
  for (int bin = 1; bin <= avg_pulse->GetN()/2; bin++){
    h1dMag->SetBinContent(bin,(2*hFFT->GetBinContent(bin)/ avg_pulse->GetN()));
  }

  TCanvas* c4 = new TCanvas("Avg Pulse FFT", "Avg Pulse FFT", 400,400);
  h1dMag->Draw();


  TCanvas* c6 = new TCanvas("Time Resolution", "Time Resolution", 1400, 400);
  c6->Divide(3,1);
  c6->cd(1); hTimeRes->Draw();
  c6->cd(2); hTimeResAll->Draw();
  c6->cd(3); hTimeResHeight->Draw("Colz");
